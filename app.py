...
def to_excel(df):
    try:
        ...
        st.error(
            "O módulo 'xlsxwriter' não está instalado. Por favor, instale-o para exportar para Excel.")
        return None
...
def main():
    ...
    data_file_1 = st.sidebar.file_uploader("Bank marketing data", type=[
                                           'csv', 'xlsx'], key='file_uploader')
    ...
    if data_file_1 is not None:
        ...
        if df_xlsx:
            st.download_button(label='📥 Download filtered table as EXCEL',
                               data=df_xlsx, file_name='bank_filtered.xlsx')
        st.markdown("---")
        ...
        if df_xlsx:
            ...
            col1.download_button(label='📥 Download',
                                 data=df_xlsx, file_name='bank_raw_y.xlsx')
        ...
        if df_xlsx:
            ...
            col2.download_button(label='📥 Download',
                                 data=df_xlsx, file_name='bank_y.xlsx')
        st.markdown("---")

        st.write('## Proporção de aceite')
        if graph_type == 'Barras':
            sns.barplot(x=bank_raw_target_perc.index, y='y',
                        data=bank_raw_target_perc, ax=ax[0])
            ax[0].bar_label(ax[0].containers[0])
            ax[0].set_title('Dados brutos', fontweight="bold")

            sns.barplot(x=bank_target_perc.index, y='y',
                        data=bank_target_perc, ax=ax[1])
